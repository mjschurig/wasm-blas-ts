...
describe('DAXPY', () => {
  ...
  test('basic operation with incx=1, incy=1', () => {
    ...
    expect(y[0]).toBeCloseTo(7); // 5 + 2*1
    expect(y[1]).toBeCloseTo(10); // 6 + 2*2
    expect(y[2]).toBeCloseTo(13); // 7 + 2*3
    expect(y[3]).toBeCloseTo(16); // 8 + 2*4
  });
  ...
  test('with negative alpha', () => {
    ...
    expect(y[0]).toBeCloseTo(7); // 10 + (-1.5)*2
    expect(y[1]).toBeCloseTo(14); // 20 + (-1.5)*4
    expect(y[2]).toBeCloseTo(21); // 30 + (-1.5)*6
  });
  ...
  test('with incx = 2', () => {
    ...
    expect(y[0]).toBeCloseTo(11); // 10 + 1*1
    expect(y[1]).toBeCloseTo(22); // 20 + 1*2
    expect(y[2]).toBeCloseTo(33); // 30 + 1*3
  });
  ...
  test('with incy = 2', () => {
    ...
    daxpy(n, alpha, x, 1, y, 2);

    expect(y[0]).toBeCloseTo(12); // 10 + 2*1
    expect(y[1]).toBeCloseTo(99); // unchanged
    expect(y[2]).toBeCloseTo(24); // 20 + 2*2
    expect(y[3]).toBeCloseTo(99); // unchanged
    expect(y[4]).toBeCloseTo(36); // 30 + 2*3
  ...
  test('handles large vectors', () => {
    ...
    for (let i = 0; i < n; i++) {
      expect(y[i]).toBeCloseTo(4.0); // 3 + 0.5*2
    }
  });
  ...
  test('handles unrolled loop (n % 4 != 0)', () => {
    ...
    expect(y[3]).toBeCloseTo(44);
    expect(y[4]).toBeCloseTo(55);
    expect(y[5]).toBeCloseTo(66);
    expect(y[6]).toBeCloseTo(77);
  });
...
describe('DAXPY - Vector Scale and Add', () => {
  ...
  it('should compute y = alpha*x + y correctly (basic case)', () => {
    ...
    const expected = new Float64Array([12, 24, 36, 48]); // y + alpha*x = [10,20,30,40] + 2*[1,2,3,4]
  ...
  it('should handle different increments', () => {
    const n = 2;
    const alpha = 3.0;
    const x = new Float64Array([1, 0, 2, 0]); // effective x = [1, 2] with incx = 2
    const y = new Float64Array([10, 0, 20, 0]); // effective y = [10, 20] with incy = 2
  ...
  it('should handle unrolled loop optimization (n >= 4)', () => {
    ...
    const expected = new Float64Array([10.5, 21, 31.5, 42, 52.5, 63, 73.5, 84]);
